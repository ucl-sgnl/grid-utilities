...
truth = SPtoMatlab('/Users/shawn/Dropbox/Satellite/PrimePoints.txt');
latitude = truth(:,1);
longitude = truth(:,2);
...
for i = 1 : Nneighbors % loop over quadratic neighbor values.
    for j = 1 : Nweights % loop over neighbor weighting values.
        gridFileName = cat(2,'Satellite',component,'-R100-Q', num2str(i+neighbors(1)-1), 'W', num2str(j+weights(1)-1), 'S0.grd');
...
dlmwrite(cat(2,'SatelliteErrors',component,'.csv'), tableData, ',') % write results to csv file.
% CSV file:
% A = neighbors
% B = weights
% C = rms
...
save(cat(2,'SatelliteErrors',component,'Workspace.mat')) % save workspace.
