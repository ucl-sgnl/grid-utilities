...
%
%% Summary:
% bilinear1.m performs bilinear interpolation on a matrix to obtain the
...
% 
% Syntax is: z = bilinear1(x,y, grid, dimensions, range)
%
%% Inputs:
%
...
% node between [ymax ymin], so that the row index corresponds to decreasing
% y-values.  
%
...
% desired point.  Then vertical interpolation (upward) is performed between
% the two horizontal points to obtain the estimate.  
%
% Note: This algorithm expects the rows of the grid to correspond to
% decreasing y-values.  If a grid M is populated so that it is appropriate
% for use in the function surf(M), then either flipud(M) must be supplied
% as the grid argument, or the y-limits may be reversed in the function
% call.  
%
%
...
function z = bilinear1(x,y, grid, dimensions, range)
...
vraw = -1*( ((y - ymax)/deltaY) - 1); % raw vertical index (downward = decreasing y-values).
...
elseif rem(hraw,1) == 0 % point falls on vertical grid line.
    ...
    y1 = ymax - (i1-1)*deltaY; % upper neighbor.
    y2 = ymax - (i2-1)*deltaY; % lower neighbor.
    f1 = grid(i1,hraw);
    f2 = grid(i2,hraw);
...
%     disp('4-point')
    ...
    i2 = fix(vraw); % lower index (smaller y-value, higher in matrix).
    i1 = i2 + 1; % higher index (greater y-value, lower in matrix).
    y1 = ymax - (i1-1)*deltaY;
    y2 = ymax - (i2-1)*deltaY;
    % Corner values
    fll = grid(i1,j1); % lower left value.
    flr = grid(i1,j2); % lower right value.
    ful = grid(i2,j1); % upper left value.
    fur = grid(i2,j2); % upper right value.
    % Linear interpolations
    R1 = ( (flr - fll)/(x2 - x1) )*(x - x1) + fll; % horizontal interpolation along lower border.
    R2 = ( (fur - ful)/(x2 - x1) )*(x - x1) + ful; % horizontal interpolation along upper border.
    z = ( (R2 - R1)/(y2 - y1) )*(y - y1) + R1; % vertical interpolation (upward) in middle of rectangle.
end % end if
